...
const buildTracingConfiguration = function (jaegerEnabled, jaegerHost, jaegerSamplerType, jaegerSamplerParam) {
  ...
  if (jaegerEnabled) {
    ...
    logger.debug(`Jaeger Sidecar Enabled: ${getJaegerAgentSidecarEnabled()}`);
  } else {
    logger.debug('Jaeger Disabled, will use opentracing.NoopTracer');
  }
...
function initTracer(serviceName) {
  const config = getTracingConfiguration();
  if (config.disabled !== true && (!getJaegerAgentSidecarEnabled() && !getJaegerHost())) {
    logger.error('JAEGER_ENABLED is set to true but no agent address or sidecar configured to send traces to.\n'
...
function setJaegerAsGlobalTracer(serviceName) {
  try {
    if (getJaegerEnabled()) {
      ...
      logger.info(`Jaeger Configured to be global tracer with service name "${serviceName}"`);
    }
  } catch (e) {
    logger.error(`Could not initialize Jaeger: ${e}. Falling back to NoopTracer`);
  }
}
